...
const admin = require('firebase-admin'); // --- NEW: FOR WALLET ---
const cors = require('cors'); // --- NEW: FOR FRONTEND CONNECTION ---
const app = express();

const PORT = process.env.PORT || 3000;

// ===================== WALLET INITIALIZATION =====================
// This uses the environment variable you will set in Railway
if (process.env.FIREBASE_SERVICE_ACCOUNT) {
  ...
  console.warn("⚠️ WARNING: FIREBASE_SERVICE_ACCOUNT not found. Wallet updates will not work.");
}
...
app.use(cors()); // --- NEW: ALLOWS FRONTEND ACCESS ---
...
// ===================== PAYSTACK WEBHOOK (WITH WALLET LOGIC) =====================
app.post('/webhook/paystack', express.raw({ type: 'application/json' }), async (req, res) => {
  ...
  if (event.event === 'charge.success') {
    ...
    const userId = metadata ? metadata.user_id : null; // --- NEW: IDENTIFY USER ---
    const depositAmount = amount / 100;

    console.log(`🔔 Webhook confirmed: ${reference} | GH₵${depositAmount}`);

    // --- NEW: UPDATE FIREBASE WALLET INSTANTLY ---
    if (userId) {
      try {
        ...
        await userWalletRef.transaction((current) => (current || 0) + depositAmount);
        ...
        console.log(`💰 WALLET CREDIT: Added GH₵${depositAmount} to user ${userId}`);
      } catch (err) {
